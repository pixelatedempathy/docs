...
<Info>
  **Prerequisites**: - Node.js v19+ (use `nvm` to manage versions) - pnpm (for
  Node.js dependencies) - uv (for Python dependencies) - Docker & Docker Compose
...
If port 5173 is already in use, the Astro dev server will attempt the next
available port. You can also specify a custom port:
...
The dev server is configured with increased memory
(`--max-old-space-size=8192`). If you still encounter memory issues:
...
[Astro Starlight](https://starlight.astro.build/). To run the docs locally:
