...
description:
  'How Pixelated Empathy implements long-term memory using Hindsight + Google Gemini
  — with PII filtering, crisis detection, speculation filtering, and
...
<CardGroup cols={2}>
  ...
  <Card title="GeminiHindsightManager" icon="brain">
    **Intelligence Layer**

    Python class that orchestrates PII filtering, speculation filtering, crisis detection, and memory storage/retrieval. Uses Google Gemini for response generation and Hindsight for vector storage.
...
## GeminiHindsightManager deep dive

The `GeminiHindsightManager` is the core orchestrator of the memory system. It
integrates Google Gemini (for PII filtering and response generation) with Hindsight
(for vector-based long-term memory storage).

### Initialization and configuration

```python
from ai.memory.hindsight_gemini.manager import GeminiHindsightManager, GeminiHindsightConfig
from ai.memory.hindsight_gemini.memory_ingestion_config import TherapeuticMemoryConfig
...
<Tip>
  The `TherapeuticMemoryConfig` uses **Hindsight cookbook best practices** for
...
<Tabs>
  <Tab title="Add Memory">
    ...
    async def add_memory(content, user_id, metadata=None, category=None):
        ...
        # Step 4: Store in Hindsight with metadata
        result = memory.add(
            filtered,
            user_id=user_id,
            metadata={
...
```text
  ...
  <Tab title="Search Memory">
    ...
    def search_memories(query, user_id):
        # Hindsight performs vector similarity search via Qdrant
...
## Custom instructions (Hindsight integration)

The memory system uses **Hindsight's custom instruction feature** to guide what gets
extracted and stored. These instructions are applied at the project level and
influence Hindsight's memory extraction behavior.
...
```text
┌─────────────────────────────────────────────────────────────┐
│  Tier 1: Gemini + Hindsight Platform Client (Production)         │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  • Hindsight Platform API (cloud-hosted vector store)    │    │
│  │  • Google Gemini API (PII filtering + generation)   │    │
│  │  • Full feature set: search, filter, custom instr.  │    │
│  └─────────────────────────────────────────────────────┘    │
│                         │ on failure                         │
│                         ▼                                    │
│  Tier 2: Local Hindsight (Self-hosted)                           │
│  ┌─────────────────────────────────────────────────────┐    │
...
<Info>
  ...
  the Hindsight Platform Client fails to initialize, the manager falls back to local
  Hindsight. If that also fails, `NullMemoryManager` is used for the lifetime of the
...
<Tip>
  ...
  URL is configured via `NEXT_PUBLIC_MEMORY_API_URL` (default:
...
| `HINDSIGHT_API_KEY`               | No       | —                       | Hindsight Platform API (cloud). If absent, uses local Hindsight |
