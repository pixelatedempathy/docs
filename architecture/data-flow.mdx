...
```text
  ...
  <Step title="Response Generation" icon="robot">
    ...
    1. Retrieve relevant memories from Foresight vector store
...
```text
    ...
    All operations pass through the `GeminiForesightManager` which enforces:
...
```text
    Primary: Google Gemini API (PII filtering + generation)
        │
        ▼ (on failure)
    Secondary: Foresight Platform Client (vector search + storage)
        │
        ▼ (on failure)
...
<Info>
  **Next:** Dive into the [Memory System
  Architecture](/architecture/memory-system) to understand how Foresight + Gemini
