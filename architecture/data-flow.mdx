...
description:
  'How data moves through Pixelated Empathy — from user input to emotional
  analysis, bias detection, memory storage, and real-time feedback. Covers all
  external integrations and communication protocols.'
---

## Data flow overview

Every interaction on the Pixelated Empathy platform triggers a precisely
orchestrated data flow across multiple services. Understanding these flows is
essential for debugging, performance optimization, and compliance auditing.

<Note>
  All data flows are designed with **HIPAA compliance** as a first-class
  constraint. PII is filtered before storage, sensitive data never leaves the
  processing boundary unencrypted, and audit logs track every data movement.
</Note>

## Training session flow

The most critical data flow in the platform is the **training session
lifecycle** — the path data takes from the moment a trainee types a message to
when they receive AI-generated feedback.
...
<Steps>
  <Step title="User Input" icon="keyboard">
    The trainee types a message in the Empathy Gym™ interface.
    The frontend captures the raw text, attaches session metadata (session ID, timestamp, scenario context), and prepares the request payload.
...
```text
  ...
  <Step title="Response Generation" icon="robot">
    ...
    1. Retrieve relevant memories from Hindsight vector store
...
Training sessions require low-latency, bidirectional communication that HTTP
request-response cannot provide. WebSocket connections handle all real-time data
flows.
...
Pixelated Empathy integrates with multiple external services and academic
databases. Each integration is abstracted behind a service interface with retry
logic, circuit breakers, and fallback behavior.
...
The Journal Research Pipeline processes academic sources through a standardized
ingestion flow:
...
The Model Context Protocol (MCP) enables external AI agents to interact with
Pixelated Empathy's memory system through a standardized interface.
...
<AccordionGroup>
  <Accordion title="What is MCP?" icon="plug">
    ...
    Pixelated Empathy implements an MCP server that exposes memory operations (add, search, update, delete) to any MCP-compatible AI agent.
...
```text
    ...
    All operations pass through the `GeminiHindsightManager` which enforces:
...
Every training session produces rich analytics data that flows through a
dedicated pipeline for reporting and continuous improvement.
...
<Warning>
  All data flows between services use **TLS encryption** in production. Internal
  service-to-service communication within the Docker/Kubernetes network uses
  mutual TLS where available. No sensitive data is transmitted in plaintext.
</Warning>
...
Each data flow implements circuit breaker patterns and fallback hierarchies to
...
```text
    Primary: Google Gemini API (PII filtering + generation)
        │
        ▼ (on failure)
    Secondary: Hindsight Platform Client (vector search + storage)
        │
        ▼ (on failure)
...
<Info>
  **Next:** Dive into the [Memory System
  Architecture](/architecture/memory-system) to understand how Hindsight + Gemini
  powers long-term memory with PII compliance, or explore the [Deployment
  Architecture](/architecture/deployment) for infrastructure details.
</Info>
