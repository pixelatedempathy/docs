...
— re-enable by uncommenting the `deploy:staging` and `health-check` jobs in `.gitlab-ci.yml`.
    </Note>
...
```bash text
    ...
    HINDSIGHT_API_KEY=your-hindsight-api-key # Legacy fallback: HINDSIGHT_API_KEY
...
<AccordionGroup>
  ...
  <Accordion title='Deployment' icon='rocket'>
    ...
    Server → Hindsight → Gemini) - [ ] Verify WebSocket connectivity for training
    sessions - [ ] Check Grafana dashboards for anomalies - [ ] Deploy to
    production with rolling update strategy
  </Accordion>
