...
Pixelated Empathy is a multi-service application. Each service handles a
distinct responsibility and communicates with others through well-defined
interfaces.
...
└────────────────────┬────────────────────────────┘
         ...
         ▼                       ▼
┌────────────────┐    ┌──────────────────┐
│  Memory (Foresight) │    │  Worker Service  │
...
The frontend is server-rendered with Astro for fast initial loads, with React
components hydrated on the client for interactive features like The Empathy Gym™
training interface.
...
The emotional intelligence engine — handles emotion analysis, client response
generation, therapeutic evaluation, and crisis detection.
...
- Scheduled maintenance tasks
  ...
  <Accordion title='Memory (Foresight)' icon='database'>
    Conversation context persistence. Maintains state across interactions so
    training scenarios can build over time.
  </Accordion>
...
We use **Zod** for runtime validation across all service boundaries. Key
validation rules:
...
1. `pnpm test:all` — Full test suite
1. `pnpm security:scan` — Security audit
