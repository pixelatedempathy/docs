...
description:
  'Get your local development environment running for Pixelated Empathy — from
  prerequisites to running all services.'
---

This guide walks you through setting up a complete local development environment
for Pixelated Empathy. By the end, you'll have the frontend, backend services,
AI engine, and supporting infrastructure running on your machine.

<Note>
  Pixelated Empathy handles sensitive mental health data. Even in local
  development, follow security practices — never commit `.env` files, never use
  real patient data in test fixtures, and always run `pnpm security:check`
  before pushing code.
</Note>
...
<AccordionGroup>
  <Accordion title="pnpm (Node.js package manager)" icon="box">
    ...
    <Warning>
      **Never use npm or yarn** for installing dependencies. The project uses pnpm's workspace features and lockfile
format. Using a different package manager will cause dependency resolution failures.
    </Warning>
  ...
  <Accordion title="uv (Python package manager)" icon="box">
    ...
    <Warning>
      **Never use pip, conda, or venv** for Python dependencies. The project uses uv's lockfile (`uv.lock`) for
...
```bash text
  ...
  <Step title="Install Node.js Dependencies">
    ...
    This installs dependencies for the main application, including Astro, React 19, TypeScript, and all backend
services.
...
```bash text
    ...
    Edit `.env.local` with your local configuration. See the [Environment Variables](#environment-variables) section
below for details.
...
The application will be available at **<http://localhost:5173>** with automatic
hot reloading as you edit files.
...
Environment variables are configured in `.env.local` for local development.
...
```bash text
    ...
    <Warning>
      Never use production keys in local development. Generate fresh secrets for your `.env.local` file. The
`security:check` script will flag any committed credentials.
    </Warning>
...
The project uses TypeScript path aliases for clean imports. These are configured
in `tsconfig.json`:
...
<AccordionGroup>
  ...
  <Accordion title="Production-Like Stack" icon="server">
    ...
    This includes the application, all backend services, and infrastructure — useful for integration testing and
...
<Steps>
  ...
  <Step title="Unit Tests">
    ...
    All tests should pass. If you see failures related to missing environment variables, ensure your `.env.local` is
...
<AccordionGroup>
  <Accordion title="Port conflicts" icon="circle-exclamation">
    If port 5173 is already in use, the Astro dev server will attempt the next available port. You can also specify a
custom port:
  ...
  <Accordion title="Node.js memory issues" icon="memory">
    The dev server is configured with increased memory (`--max-old-space-size=8192`). If you still encounter memory
issues:
...
<CardGroup cols={2}>
  <Card
    ...
    icon='code'
    href='/guides/developers/conventions'
  >
    Learn the coding standards, file naming conventions, and architectural
  ...
  >
    Browse the API endpoints for sessions, analytics, bias detection, and crisis
    detection.
  </Card>
</CardGroup>
````
